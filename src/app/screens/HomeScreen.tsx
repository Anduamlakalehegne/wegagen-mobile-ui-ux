  ...
  return (
    <>
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-[#FF8F12] to-[#ff6b0b] opacity-95">
        ...
        <div className="absolute inset-0 opacity-10 bg-[radial-gradient(circle_at_20%_50%,rgba(255,255,255,0.8),transparent_50%)]"></div>
        <div className="absolute inset-0 opacity-5 bg-[linear-gradient(45deg,transparent_25%,rgba(0,0,0,0.1)_25%,rgba(0,0,0,0.1)_50%,transparent_50%,transparent_75%,rgba(0,0,0,0.1)_75%)] bg-[length:40px_40px]"></div>
      </div>
    </>
  );
}
  ...
  return (
    <button
      ...
      className={`flex flex-col items-center gap-3 p-4 rounded-2xl transition-all duration-300 ${
        isActive
          ? 'bg-[#FF8F12] text-white shadow-lg scale-105'
          : 'bg-[#FFF6EC] text-[#004360] hover:shadow-md active:scale-95'
      }`}
    >
      <div className={`p-3 rounded-lg ${isActive ? 'bg-white/20' : 'bg-white/60'}`}>
        {icon}
      </div>
...
function TransferOption({ icon, label, onClick }: TransferOptionProps) {
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-4 p-4 bg-[#FFF6EC] rounded-xl hover:bg-[#FFEDE0] transition-all duration-200 active:scale-[0.98]"
    >
      <div className="w-12 h-12 rounded-lg bg-white flex items-center justify-center text-[#FF8F12]">
        {icon}
      </div>
      <span className="text-[#004360] font-medium flex-1 text-left">{label}</span>
      <ChevronDown size={20} className="text-[#999] rotate-[-90deg]" />
    </button>
  );
}
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="bg-white relative w-[412px] h-[917px] overflow-hidden shadow-2xl">
        ...
        <div className="absolute h-[260px] left-0 top-0 w-full">
          ...
          {/* Balance Cards */}
          <div className="relative z-10 px-6 pt-4 space-y-4">
            {/* Savings Card */}
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
              <p className="text-white/80 text-sm font-medium mb-2">Saving:</p>
        ...
        <div className="absolute top-[390px] left-0 right-0 bottom-[90px] overflow-y-auto px-6 pt-6 pb-4">
          ...
          <div className="mt-6 bg-gradient-to-br from-gray-100 to-gray-200 rounded-xl p-8 text-center min-h-[140px] flex flex-col items-center justify-center shadow-sm">
            <p className="text-gray-600 italic text-lg font-light">Advertisement</p>
          </div>
        </div>
